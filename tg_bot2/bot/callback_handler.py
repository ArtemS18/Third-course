...
class CallbackHandlers():
    ...
    def callback_settings(self, call):
        ...
        if value == "price-range":
            ...
            text_price = f"Цена от {self.user.get_data_filters(call.from_user.id)['min_price_filter']}"
            if self.user.get_data_filters(call.from_user.id)['max_price_filter']:
                text_price += f" и до {self.user.get_data_filters(call.from_user.id)['max_price_filter']}"
